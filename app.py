...
def generate_response(user_message):
    ...
    # 菜單/產品查詢
        ...
        if website_info:
            if website_info['products']:
                products_text = "\n• ". join(website_info['products'][:8])  # 顯示前8個項目
                return f"🍵 {website_info['title']}\n\n我們的產品：\n• {products_text}\n\n更多資訊請訪問：\n{WEBSITE_URL}"
            else:
                return f"🍵 {website_info['title']}\n\n{website_info['description']}\n\n詳細資訊請訪問：\n{WEBSITE_URL}"
        else: 
            return f"😅 抱歉，目前無法取得網站資訊。\n\n請直接訪問我們的官網：\n{WEBSITE_URL}"
    
    # 關於/介紹
    elif any(keyword in message_lower for keyword in ['about', '關於', '介紹', '簡介']):
        website_info = fetch_website_info()
        if website_info: 
            return f"📖 {website_info['title']}\n\n{website_info['description']}\n\n了解更多：\n{WEBSITE_URL}"
        else: 
